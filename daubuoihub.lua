...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('Skid hub Beta') then
        makefolder('Skid Hub Beta')
    end
    if not _G.Dis and not isfile('Skid hub Beta/'..Name..'.json') then
        writefile('Skid hub Beta/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('Skid hub Beta') and isfile('Skid hub Beta/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('Skid hub Beta/'..Name..'.json'))
...
function LoadSettings()
    if readfile and writefile and isfile and isfolder then
        if not isfolder("Skid hub Beta") then
            makefolder("Skid hub Beta")
        end
        if not isfolder("Skid hub Beta/Blox Fruits/") then
            makefolder("Skid hub Beta/Blox Fruits/")
        end
        if not isfile("Skid hub Beta/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
            writefile("Skid hub Beta/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(_G.Settings))
        else
            local Decode = game:GetService("HttpService"):JSONDecode(readfile("Skid hub Beta/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"))
...
function SaveSettings()
    if readfile and writefile and isfile and isfolder then
        if not isfile("Skid hub Beta/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
            ...
            writefile("Skid hub Beta/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(Array))
        end
    else
        return warn("Status : Undetected Executor")
    end
...
local Library = Update:Window("Skid hub Beta","2.0",Enum.KeyCode.RightControl);
